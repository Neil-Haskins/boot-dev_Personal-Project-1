...
def window(width, height, colors=None):
    ...
    month_start_select_box.grid(column=2, row=1, sticky=(W, E))
    ...
    month_end_select_box.grid(column=2, row=2, sticky=(W, E))
    ...
    filter_select_options = StringVar()
    filter_select_options.set('Select filter')
    filter_select_box = OptionMenu(main_frame, filter_select_options, *filters.filters_dict.keys())
    filter_select_box.grid(column=2, row=3, sticky=(W, E))
    ...
    filter_value_label.grid(column=1, row=4, sticky=(W, E))

    filter_select_options.trace('w', lambda *args : filter_value_label_text.set(f'{filter_select_options.get()} = '))
    ...
    find_button = ttk.Button(main_frame, text='Find cities', command=lambda *args : change_dropdown(filter_value))
